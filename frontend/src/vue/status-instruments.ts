...
export const RovBatteryVue = defineComponent({
  ...
  setup() {
    const value = ref<string | null>(null); let unsubscribe: (() => void) | undefined;
    onMounted(() => { const store = window.rovCockpitTelemetry; if (store) unsubscribe = store.subscribe(update => { value.value = update.state.values.get(TOPICS.batteryPercentage) ?? null; }); });
    onBeforeUnmount(() => unsubscribe?.());
    return () => {
      if (value.value === null || !Number.isFinite(Number(value.value))) return h("div", { class: ["rov-instrument", "rov-instrument--unavailable", "rov-battery"], role: "status", "aria-live": "polite" }, [h("i", { class: "fa-solid fa-battery-empty", "aria-hidden": "true" }), h("span", { class: "rov-instrument__value" }, "Unavailable")]);
