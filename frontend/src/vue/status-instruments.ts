...
export const RovBatteryVue = defineComponent({
  ...
  setup() {
    ...
    onBeforeUnmount(() => unsubscribe?.());
    return () => {
      if (value.value === null || !Number.isFinite(Number(value.value))) return h("div", { class: ["rov-instrument", "rov-instrument--unavailable", "rov-battery"], role: "status", "aria-live": "polite" }, [h("i", { class: "fa-solid fa-battery-empty", "aria-hidden": "true" }), h("span", { class: "rov-instrument__value" }, "-- %")]);
