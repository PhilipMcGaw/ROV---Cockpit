...
function valid(reading: NumericTelemetry | undefined): reading is NumericTelemetry & { value: number } {
...
export class RovHud extends HTMLElement {
    ...
    this.innerHTML = `
      ...
      <div class="rov-hud__attitude">
        <svg viewBox="0 0 240 240" aria-hidden="true">
          ...
          <circle class="rov-hud__bezel" cx="120" cy="120" r="108" />
          <g clip-path="url(#rov-hud-clip)"><g data-horizon>
            <rect class="rov-hud__sky" x="0" y="0" width="240" height="120" />
            <rect class="rov-hud__water" x="0" y="120" width="240" height="120" />
            <line class="rov-hud__horizon-line" x1="0" y1="120" x2="240" y2="120" />
            <path class="rov-hud__marks" d="M65 90h35 M75 105h25 M65 150h35 M75 135h25 M175 90h-35 M165 105h-25 M175 150h-35 M165 135h-25" />
          </g></g>
          <path class="rov-hud__aircraft" d="M120 92v34 M94 116h52 M106 116l14 10 14-10" />
          <circle class="rov-hud__centre" cx="120" cy="120" r="3" />
        </svg>
        <output class="rov-hud__roll" data-roll>r: --</output>
  ...
  private consume(update: TelemetryStateUpdate): void {
    ...
    const roll = numeric.get(TOPICS.roll);
    const pitch = numeric.get(TOPICS.pitch);
    const depth = numeric.get(TOPICS.depth);
    const heading = numeric.get(TOPICS.heading);
    const horizon = this.querySelector<SVGGElement>("[data-horizon]");
    const rollValue = valid(roll) ? roll.value : null;
    const pitchValue = valid(pitch) ? pitch.value : null;
    if (horizon) horizon.setAttribute("transform", `rotate(${rollValue ?? 0} 120 120) translate(0 ${pitchValue === null ? 0 : pitchValue * 0.7})`);
