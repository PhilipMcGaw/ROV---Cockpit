...
const numberValue = (value, unit, precision = 1) => {
...
export const RovTelemetryConnectionVue = defineComponent({
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            if (store)
                unsubscribe = store.subscribe(update => { connected.value = update.state.connected; });
        });
...
export const RovLiveDockVue = defineComponent({
    ...
    setup() {
        ...
        const lightPercentage = ref(null);
        const cameraPitch = ref(null);
        ...
        onBeforeUnmount(() => unsubscribe?.());
        const metric = (icon, label, value) => h("div", { class: "rov-command-dock__metric" }, [
...
//# sourceMappingURL=live-dock.js.map