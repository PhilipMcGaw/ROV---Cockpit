...
export class RovAttitude extends HTMLElement {
    ...
    pitchValue;
    connectedCallback() {
        this.classList.add("rov-attitude");
        this.setAttribute("role", "img");
        this.setAttribute("aria-label", "ROV pitch and roll attitude");
        ...
        this.horizon = this.querySelector(".rov-attitude__horizon");
        this.rollValue = this.querySelector(".rov-attitude__roll");
        this.pitchValue = this.querySelector(".rov-attitude__pitch");
    ...
    consume(update) { const pitch = update.state.numeric.get(TOPICS.pitch); const roll = update.state.numeric.get(TOPICS.roll); const p = this.valid(pitch) ? pitch.value : null; const r = this.valid(roll) ? roll.value : null; if (this.horizon)
        this.horizon.setAttribute("transform", `rotate(${r ?? 0} 120 120) translate(0 ${p === null ? 0 : p * 0.7})`); if (this.rollValue)
        this.rollValue.textContent = r === null ? "r: --" : `r: ${r.toFixed(2)}`; if (this.pitchValue)
        this.pitchValue.textContent = p === null ? "p: --" : `p: ${p.toFixed(2)}`; this.dataset.status = p === null || r === null ? "unavailable" : "available"; }
    valid(reading) { return Boolean(reading?.valid && reading.value !== null && Number.isFinite(reading.value)); }
}
