...
export class RovHud extends HTMLElement {
    ...
    drawAttitude(roll, pitch) { const canvas = this.querySelector(".rov-hud__attitude canvas"), ctx = canvas?.getContext("2d"); if (!canvas || !ctx)
        ...
        ctx.fillText(String(angle), radius * 1.95, y - 4);
    } ctx.setLineDash([]); ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, radius, Math.PI * .75, Math.PI * 1.25); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, radius, -Math.PI * .25, Math.PI * .25); ctx.stroke(); ctx.beginPath(); ctx.moveTo(-radius, 0); ctx.lineTo(-radius * 1.5, 0); ctx.moveTo(radius, 0); ctx.lineTo(radius * 1.5, 0); ctx.stroke(); ctx.strokeStyle = getComputedStyle(this).getPropertyValue("--hud-accent") || "#ffb000"; ctx.beginPath(); ctx.moveTo(-radius * .75, 0); ctx.lineTo(-radius * .25, 0); ctx.moveTo(radius * .25, 0); ctx.lineTo(radius * .75, 0); ctx.moveTo(0, -radius * .2); ctx.lineTo(0, radius * .2); ctx.stroke(); ctx.restore(); }
