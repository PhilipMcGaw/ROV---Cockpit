...
export class RovHud extends HTMLElement {
    ...
    updateHeadingTape() { this.querySelectorAll("[data-angle]").forEach(item => { const relative = wrap180(Number(item.dataset.angle) - this.heading); item.style.transform = `translateX(calc(-50% + ${relative * 4}px))`; item.style.opacity = Math.abs(relative) > 25 ? "0" : "1"; }); }
