...
The HUD heading tape keeps the current heading centred beneath its amber marker. It uses 5° tick spacing and displays a ±25° window around the current heading. The centre is labelled numerically as `0°`; cardinal ticks retain their cardinal styling. Each heading tick is centred on its own position before the relative bearing offset is applied, preventing heading labels and tick marks from overlapping.
