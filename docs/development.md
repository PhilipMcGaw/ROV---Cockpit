...
The attitude reference geometry is drawn when the HUD mounts, even if telemetry is unavailable; unavailable roll and pitch values do not collapse the instrument layout.
The attitude arc is rendered as matching left and right segments around the central amber reference marker.
