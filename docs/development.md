...
The attitude arc is rendered as matching left and right segments. No amber or yellow aircraft reference marks are drawn inside the arcs. The attitude instrument displays only the solid centre horizontal line on each side, with its inner ends touching the arcs; no other pitch lines are drawn there. The outside roll scales are vertically offset so their `0` lines align with, or sit slightly below, the centre lines.
